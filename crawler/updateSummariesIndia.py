...
def main():
    ...
    while aTest:
        ...
        myCursor.execute("SELECT link, title, article FROM indianews WHERE summary is NULL LIMIT 10")
        ...
        for i in range(len(df)):
            sql = ("UPDATE indianews SET summary = '" + df['summarized'][i] + "' WHERE link LIKE '" + df['link'][i] + "'")
